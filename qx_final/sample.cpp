...
const char* WINDOWTITLE = { "Final Project -- Raymond" };
...
void DrawCirclea(float r)
{
	glColor3f(1., 1., 1.);
	glBegin(GL_LINE_STRIP);
	for (int i = 0; i <= 100; i++)
	{
		glVertex3f(r * cos((2 * M_PI * i) / 100), 10., r * sin((2 * M_PI * i) / 100));
	}
	glEnd();
}

void DrawCircleb(float r)
{
	glColor3f(1., 1., 1.);
	glBegin(GL_LINE_STRIP);
	for (int i = 0; i <= 100; i++)
	{
		glVertex3f(r * cos((2 * M_PI * i) / 100), 20., r * sin((2 * M_PI * i) / 100));
	}
	glEnd();
}
...
Animate()
{
	...
	if ((ms % 16) % 4 == 0)
	{
		DiscoLight1Dirx = random_gen(-30, 30);
		DiscoLight1Diry = random_gen(0, 30);
		DiscoLight1Dirz = random_gen(-30, 30);
		DiscoLight2Dirx = random_gen(-30, 30);
		DiscoLight2Diry = random_gen(0, 30);
		DiscoLight2Dirz = random_gen(-30, 30);
		DiscoLight3Dirx = random_gen(-30, 30);
		DiscoLight3Diry = random_gen(0, 30);
		DiscoLight3Dirz = random_gen(-30, 30);
		/*DiscoLight4Dirx = random_gen(-30, 30);
		DiscoLight4Diry = random_gen(0, 30);
		DiscoLight4Dirz = random_gen(-30, 30);*/
	}
	...
	if (ms % 20 == 0)
	{
		...
		{
			snowmove[snowgen][i][0] = random_gen(-2, 3);
			snowmove[snowgen][i][1] = random_gen(-2, 3);
			snowpos[snowgen][i][0] = random_gen(-50, 50);
			snowpos[snowgen][i][1] = random_gen(30, 40);
			snowpos[snowgen][i][2] = random_gen(-50, 50);
...
Display()
{
	...
	//disco lights
	//spotlight
	SetSpotLight(GL_LIGHT1, 10., -sqrt(42.5 * 42.5 - 27. * 27.) - 7.5, 15., 0, 10, 10, 1., 0., 0.);
	//point light
	SetSpotLight(GL_LIGHT3, 10., -sqrt(42.5 * 42.5 - 27. * 27.) - 7.5, -10., 0, 10, 10, 0., 1., 0.);
	SetSpotLight(GL_LIGHT4, -10., -sqrt(42.5 * 42.5 - 27. * 27.) - 7.5, -10., 0, 10, 10, 0., 0., 1.);
	//SetSpotLight(GL_LIGHT5, -10., -sqrt(42.5 * 42.5 - 27. * 27.) - 7.5, 10., DiscoLight4Dirx, DiscoLight4Diry, DiscoLight4Dirz, 1., 1., 0.);
	...
	else if (DiscoSwitch == 1)
	{
		...
		glEnable(GL_LIGHT4);
		glEnable(GL_LIGHT5);
		if (DiscoParaSwitch == 1)
		{
			glEnable(GL_LIGHT6);
		}
		else if (DiscoParaSwitch == 0)
		{
			glDisable(GL_LIGHT6);
		}
	}
	...
	else if (DiscoSwitch == 1)
	{
		glRotatef(360 * Time, 0., 1., 0.);
		glCallList(SunList);
		SetPointLight(GL_LIGHT0, 0., 0., 0., 0., 0., 1.);
	}
	glPopMatrix();
	...
	glEnable(GL_LIGHTING);
	glPushMatrix();
	glRotatef(360 * Time * sqrt(cude_cal(38.)) / sqrt(cude_cal(18.25)), 0., 2., 0.);
	glRotated(Earth_orig, 0, 2, 0);
	glTranslatef(18.25, 10., 0.);
	glRotatef(360 * Earth_orig / 10 * Time, 0., 2., 0.);
	SetMaterial(1.0, 1.0, 1.0, 2.);
	...
	glEnable(GL_LIGHTING);
	glPushMatrix();
	glRotatef(360 * Time * sqrt(cude_cal(38.)) / sqrt(cude_cal(1.5625)), 0., 2., 0.);
	glRotated(Moon_orig, 0, 2, 0);
	glTranslatef(-1.5625, 10., 0.);
	glRotatef(360 * Moon_orig/10 * Time, 0., 2., 0.);
	SetMaterial(1.0, 1.0, 1.0, 2.);
	glCallList(MoonList);
	glPopMatrix();
	//glPopMatrix();
	...
	glPushMatrix();
	glRotatef(360 * Time * sqrt(cude_cal(38.)) / sqrt(cude_cal(18.25)), 0., 2., 0.);
	glRotated(Jupiter_orig, 0, 2, 0);
	glTranslatef(18.25, 10., 0.);
	glRotatef(360 * Jupiter_orig / 10 * Time, 0., 2., 0.);
	SetMaterial(1.0, 1.0, 1.0, 2.);
	...
	glPushMatrix();
	glRotatef(360 * Time * sqrt(cude_cal(38.)) / sqrt(cude_cal(18.25)), 0., 2., 0.);
	glRotated(Saturn_orig, 0, 2, 0);
	glTranslatef(18.25, 10., 0.);
	glRotatef(40., -1., 0., 1.);
	glRotatef(360 * Saturn_orig / 10 * Time, 0., 2., 0.);
	SetMaterial(1.0, 1.0, 1.0, 2.);
	...
	glEnable(GL_LIGHTING);
	glPushMatrix();
	glRotatef(360 * Time * sqrt(cude_cal(38.)) / sqrt(cude_cal(36.49)), 0., -4., 0.);
	glRotated(Uranus_orig, 0, -4, 0);
	glTranslatef(36.49, 20., 0.);
	glRotatef(70., 1., 0.35, 0.25);
	glRotatef(360 * Uranus_orig / 10 * Time, 0., -4., 0.);
	SetMaterial(1.0, 1.0, 1.0, 2.);
	...
	glPushMatrix();
	glRotatef(360 * Time * sqrt(cude_cal(38.)) / sqrt(cude_cal(36.49)), 0., -4., 0.);
	glRotated(Neptune_orig, 0, -4, 0);
	glTranslatef(36.49, 20., 0.);
	glRotatef(360 * Neptune_orig / 10 * Time, 0., -4., 0.);
	SetMaterial(1.0, 1.0, 1.0, 2.);
	...
	glPushMatrix();
	glRotatef(360 * Time * sqrt(cude_cal(38.)) / sqrt(cude_cal(36.49)), 0., -4., 0.);
	glRotated(Pluto_orig, 0, -4, 0);
	glTranslatef(36.49, 20., 0.);
	glRotatef(360 * Pluto_orig / 10 * Time, 0., -4., 0.);
	SetMaterial(1.0, 1.0, 1.0, 2.);
	...
	if (SnowSwitch == 1)
	{
		...
		{
			for (int q = 0; q < snowgen; q++)
			{
				snowpos[q][j][0] += 2*Time * snowmove[q][j][0];
				snowpos[q][j][1] += 4*Time * snowfall[q][j];
				snowpos[q][j][2] += 6*Time * snowmove[q][j][1];
				if (sqrt(snowpos[q][j][0] * snowpos[q][j][0] + snowpos[q][j][1] * snowpos[q][j][1] + snowpos[q][j][2] * snowpos[q][j][2]) <= 38)
				{
					glPushMatrix();
					glTranslatef(snowpos[q][j][0], snowpos[q][j][1], snowpos[q][j][2]);
...
InitMenus()
{
	...
	glutAddSubMenu("Point Lighting Effect", discomenu);
	glutAddSubMenu("Meteors", snowmenu);
...
InitGraphics()
{
	...
	//Wood Texture
	/*Texture = BmpToTexture("WoodTex.bmp", &Width, &Height);
	if (Texture == NULL || Width != 1024 || Height != 512)
	{
		printf("Wrong Reading BMP File!");
		exit(1);
	...
	//Velvet Texture
	Texture = BmpToTexture("velvetmap.bmp", &Width, &Height);
	...
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	glGenTextures(1, &VelvetTex);
	glBindTexture(GL_TEXTURE_2D, VelvetTex);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
