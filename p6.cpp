...
const char* WINDOWTITLE = { "Raymond Yuan--Geometric Modeling" };
...
void Display() {
    ...
    gluLookAt(0.01, 5, 0., 0., 0., 0., 0., 1., 0.);
    ...
    for (int i = 0; i < 6; i++) {
        bar[i].color(0, 0, 0);
        RotateY(&bar[i].p0, (bar[i].p0.y / HELIXHEIGHT) * 90 * sinf(1), 0, 0, 0);
        RotateY(&bar[i].p3, (bar[i].p0.y / HELIXHEIGHT) * 90 * sinf(1), 0, 0, 0);
        DrawBezierCurve(bar[i], 3);
    }

    for (int i = 6; i < NUMBARS; i++) {
        bar[i].color(0.65, 0.22, 0.13);

        RotateY(&bar[i].p0, (bar[i].p0.y / HELIXHEIGHT) * 180 * sinf(Time), 0, 0, 0);
        RotateY(&bar[i].p3, (bar[i].p0.y / HELIXHEIGHT) * 180 * sinf(Time), 0, 0, 0);
        //RotateY(&bar[i].p1, (bar[i].p0.y/HELIXHEIGHT)*180*sinf(Time), 0, 0, 0);
        //RotateY(&bar[i].p2, (bar[i].p0.y/HELIXHEIGHT)*180*sinf(Time), 0, 0, 0);
        DrawBezierCurve(bar[i], 3);
    }
...
void InitGraphics() {
    ...
    for (int i = 0; i < NUMBARS; ++i) {
        bar[i].color(0.7, 0.3, 0.1 + (float)i / NUMBARS);
        barheight = (((float)i / (NUMBARS - 1)) * 8 * (HELIXHEIGHT - 0.1));
