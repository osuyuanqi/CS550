...
const char *WINDOWTITLE = { "OpenGL / GLUT Sample -- Qingxiao Graphics" };
...
Display( )
{
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., (char *)"Project #1 Qingxiao Yuan's ice cream" );
...
InitLists()
{
	...
	float x, y, z;
	float R1 = 0.4, R2 = 0.6, R3 = 0.8, R4 = 1.0;
	...
	for (int i = 0; i <= 100; i++)
	{
		glVertex3f(R4 * cos(0.02 * M_PI * i), -0.4, R4 * sin(0.02 * M_PI * i));
	}
	glEnd();
	...
	for (int i = 0; i <= 100; i++)
	{
		glVertex3f(R2 * cos(0.02 * M_PI * i), -0.2, R2 * sin(0.02 * M_PI * i));
	}
	glEnd();
	...
	for (int i = 0; i <= 100; i++)
	{
		glVertex3f(R4 * cos(0.02 * M_PI * i), -0.6, R4 * sin(0.02 * M_PI * i));
	}
	glEnd();
