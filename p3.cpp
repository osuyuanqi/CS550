...
const char *WINDOWTITLE = { "Qingxiao(Raymond) Yuan - #Project 3" };
...
void MjbSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
		...
		for (int ilng = 0; ilng < NumLngs; ilng++)
		{
			...
			if (Distort)
			{
				p->s = (lng + M_PI) / (2*M_PI) + sin(2.*(lat - M_PI/2. + TimeCo))*1.5;
				//p->t = ? ? ? ? ?
				p->t = (lat + M_PI / 1.8) / M_PI;
			}
			else
			{
				p->s = (lng + M_PI) / (2.*M_PI);
...
Display( )
{
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "Raymond Yuan - Texture Mapping" );
...
InitMenus( )
{
	...
	glutAddMenuEntry("blob-ish object", 0 );
